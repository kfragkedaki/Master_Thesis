...
def generate_evrp_data(dataset_size, graph_size, num_trailers, num_trucks):
    coordinates = np.random.uniform(size=(dataset_size, graph_size, 2)).tolist()
    node_chargers = np.random.randint(low=1, high=10, size=(dataset_size, graph_size)).tolist()
    trucks_locations = [np.random.choice(graph_size, num_trucks, replace=False).tolist() for _ in range(dataset_size)]
    trucks_battery_levels = np.ones(shape=(dataset_size, num_trucks)).tolist()
    trailers_locations = [np.random.choice(graph_size, num_trailers, replace=False).tolist() for _ in range(dataset_size)]
    trailers_status = np.ones(shape=(dataset_size, num_trailers)).tolist()
    destinations = [[np.random.choice([n for n in range(graph_size) if n != node_id]).tolist() for node_id in trailer_origins]
                       for trailer_origins in trailers_locations]
    start_time = np.random.randint(low=8.00, high=18.00, size=(dataset_size, num_trailers))
    end_time = (start_time + np.round(np.random.uniform(low=0.5, high=2, size=(dataset_size, num_trailers)) * 2) / 2)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--name", type=str, default='new2',help="Name to identify dataset")
    ...
    parser.add_argument('--graph_sizes', type=int, nargs='+', default=[4, 20],
                        help="Sizes of problem instances (default 20, 50, 100)")
    parser.add_argument('--num_trucks', type=int, nargs='+', default=2,
                        help="Size of the fleet")
    parser.add_argument('--num_trailers', type=int, nargs='+', default=3,
