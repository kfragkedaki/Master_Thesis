...
def plot_timeline(data: np.array = []) -> None:
    assert isinstance(data, np.ndarray) or (len(data) > 0 and len(data[0]) > 0), "Wrong data type"

    df = pd.DataFrame(columns=['Truck', 'StartTime', 'FinishTime', 'StartNode', 'EndNode', 'Trailer'])
        ...
        for idx, row in truck_df.iterrows():
            fig.add_trace(
                go.Scatter(
                    ...
                    mode='lines',
                    name=truck,
                    line=dict(width=20, color=trailer_colors[trailer_names.index(row['Trailer'])]),
                    hoverinfo='none',
                    showlegend=False
                )
            )
    ...
    for i, trailer_name in enumerate(trailer_names):
        legend_data.append(go.Scatter(x=[None], y=[None], mode='markers', marker=dict(color=trailer_colors[i]),
                                    name=trailer_name, legendgroup=trailer_name))
