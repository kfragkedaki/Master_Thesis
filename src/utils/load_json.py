...
def validate_data(data):
    ...
    for i, trailer in enumerate(data['trailers']):
        if trailer['id'] != f'Trailer {i}':
            return False
        if trailer['originNodeId'] not in node_ids or trailer['destinationNodeId'] not in node_ids:
            return False
...
def trasform_data(data: dict):
    ...
    for node in data['nodes']:
        transformed_data[node['id']] = {
            ...
            'available_chargers': node['num_chargers'],  # all chargers are available initially
            'trailers': {},
            'trucks': {}
        }
    ...
    for truck in data['trucks']:
        ...
        truck_info = {
            'battery_level': truck['startBatteryLevel']
        }
