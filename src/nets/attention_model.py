...
writer = SummaryWriter(log_dir="logs/plots/" + datetime.now().strftime("%Y%m%d-%H%M%S"))
...
class AttentionModel(nn.Module):
    ...
    def forward(self, input, return_pi=False):
        ...
        # input_distance_3d = torch.stack([input_distance] * 3)

        # Log the tensor as an image
        # writer.add_image('input_distance', input_distance_3d, dataformats='CHW')

        # Create a test image (a 10x10 white image)
        # test_image = torch.ones(3, 10, 10)
        # writer.add_image('test_image', test_image, dataformats='CHW')
        # writer.close()
